...
compileTest("transpileFile kv stdlib output compiles with the available C++ compiler", (t) => {
  const targetDir = createManagedTempDir(t, "kv-compile");
  ...
  assert.ok(cppFiles.some((file) => file.includes("stdlib_jayess_kv_index_js.cpp")));
});
