...
compileTest("transpileFile test stdlib output compiles with the available C++ compiler", (t) => {
  const targetDir = createManagedTempDir(t, "test-stdlib-compile");
  ...
  assert.ok(cppFiles.some((file) => file.includes("stdlib_jayess_test_index_js.cpp")));
});
