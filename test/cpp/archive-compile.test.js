...
compileTest("transpileFile archive module output compiles with the available C++ compiler", (t) => {
  const targetDir = createManagedTempDir(t, "archive-compile");
  ...
  assert.ok(cppFiles.some((file) => file.includes("stdlib_jayess_archive_index_js.cpp")));
});
