...
compileTest("transpileFile config stdlib output compiles with the available C++ compiler", (t) => {
  const targetDir = createManagedTempDir(t, "config-stdlib-compile");
  ...
  assert.ok(cppFiles.some((file) => file.includes("stdlib_jayess_config_index_js.cpp")));
});
