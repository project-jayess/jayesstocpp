...
compileTest("transpileFile cookie stdlib output compiles with the available C++ compiler", (t) => {
  const targetDir = createManagedTempDir(t, "cookie-compile");
  ...
  assert.ok(cppFiles.some((file) => file.includes("stdlib_jayess_cookie_index_js.cpp")));
});
