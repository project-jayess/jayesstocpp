...
compileTest("transpileFile subprocess output compiles with the available C++ compiler", (t) => {
  const targetDir = createManagedTempDir(t, "subprocess-compile");
  ...
  assert.ok(cppFiles.some((file) => file.includes("stdlib_jayess_subprocess_index_js.cpp")));
});
