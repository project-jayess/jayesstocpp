...
compileTest("transpileFile validate module output compiles with the available C++ compiler", (t) => {
  const targetDir = createManagedTempDir(t, "validate-compile");
  ...
  assert.ok(cppFiles.some((file) => file.includes("stdlib_jayess_validate_index_js.cpp")));
});
