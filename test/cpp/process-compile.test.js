...
compileTest("transpileFile process output compiles with the available C++ compiler", (t) => {
  const targetDir = createManagedTempDir(t, "process-compile");
  ...
  assert.ok(cppFiles.some((file) => file.includes("stdlib_jayess_process_index_js.cpp")));
});
