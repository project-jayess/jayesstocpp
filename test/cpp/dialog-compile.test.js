...
compileTest("transpileFile dialog stdlib output compiles with guarded platform adapters", (t) => {
  const targetDir = createManagedTempDir(t, "dialog-compile");
  ...
  assert.ok(cppFiles.some((file) => file.includes("stdlib_jayess_dialog_index_js.cpp")));
});
