...
test("transpileFile emits window module runtime and native bridge output", (t) => {
  ...
  assert.match(headerSource, /value window_hide\(const value& window\);/);
  assert.match(headerSource, /value window_frame\(const value& window, const value& enabled\);/);
  ...
  assert.match(cppSource, /window_platform_hide/);
  assert.match(cppSource, /window_platform_frame/);
  ...
  assert.match(cppSource, /window_windows_fill_black/);
  ...
  assert.match(cppSource, /blackColor/);
  assert.match(cppSource, /setBackgroundColor:/);
  ...
  assert.match(cppSource, /_MOTIF_WM_HINTS/);
  assert.match(cppSource, /XChangeProperty/);
  ...
  assert.match(cppSource, /if \(!window->shown\) \{\s*return windowValue;\s*\}/s);
  ...
  assert.match(cppSource, /0x000000/);
  assert.doesNotMatch(cppSource, /0xffffff/);
  ...
  assert.match(cppSource, /window_get_property/);
  assert.match(cppSource, /window_frame/);
  assert.match(cppSource, /window_add_event_listener/);
  assert.match(cppSource, /window_dispatch_events/);
  assert.match(cppSource, /window_run/);
  assert.match(cppSource, /window_set_fps/);
  assert.match(cppSource, /window_current_fps/);
  assert.match(cppSource, /window_request_render/);
  assert.match(cppSource, /Jayess window frame flag must be a boolean/);
...
test("transpileFile emits window handle method surface output", (t) => {
  const targetDir = createManagedTempDir(t, "window-method-output");
  const fixture = path.resolve("test/fixtures/modules/window-method-main.js");
  transpileFile(fixture, targetDir);

  const cppSource = fs.readFileSync(path.join(targetDir, "runtime", "jayess_runtime.cpp"), "utf8");
  const entrySource = fs.readFileSync(path.join(targetDir, "window_method_main_js.cpp"), "utf8");

  assert.match(cppSource, /value get_native_property/);
  assert.match(cppSource, /window_get_property\(input, key\)/);
  assert.match(cppSource, /key == "addEventListener"/);
  assert.match(cppSource, /key == "hide"/);
  assert.match(cppSource, /key == "frame"/);
  assert.match(cppSource, /key == "dispatchEvents"/);
  assert.match(cppSource, /key == "run"/);
  assert.match(cppSource, /key == "setFps"/);
  assert.match(cppSource, /key == "currentFps"/);
  assert.match(cppSource, /key == "requestRender"/);
  assert.match(cppSource, /key == "isClosing" \|\| key == "shouldClose"/);
  assert.match(entrySource, /jayess::get_property\(window, "addEventListener"\)/);
  assert.match(entrySource, /jayess::get_property\(window, "frame"\)/);
  assert.match(entrySource, /jayess::get_property\(window, "dispatchEvents"\)/);
  assert.match(entrySource, /jayess::get_property\(window, "setFps"\)/);
  assert.match(entrySource, /jayess::get_property\(window, "requestRender"\)/);
  assert.match(entrySource, /jayess::get_property\(window, "isClosing"\)/);
  assert.match(entrySource, /jayess::get_property\(window, "currentFps"\)/);
});
