...
Completed milestones through section 424 now live here.
...
## 424. External Image Decoder Support For `jayess:image` And `jayess:canvas`

- [x] Detect bundled decoder sources under `/externals/stb` and copy required decoder source plus license notices into generated projects that retain the image runtime.
- [x] Add a focused C++ adapter boundary in `src/cpp/runtime-image-decode-source.js` instead of adding PNG/JPEG/GIF/PSD logic to the existing image buffer or file-format runtime files.
- [x] Define one STB-backed adapter path that decodes encoded bytes into Jayess RGBA pixels.
- [x] Wire `jayess:image` runtime declarations for `decodePng`, `decodeJpeg`, `decodePsd`, `decodeGif`, `loadPng`, `loadJpeg`, `loadJpg`, `loadPsd`, `loadGif`, and `loadImage`.
- [x] Keep existing `loadBmp` available while allowing `loadImage` and XML image loading to route `.bmp` through the generic STB-backed extension dispatcher.
- [x] Add a generic `loadImage(path)` helper in `jayess:image` that dispatches by extension across `.ppm`, `.pgm`, `.bmp`, `.tga`, `.png`, `.jpeg`, `.jpg`, `.psd`, `.gif`, and `.webp`.
- [x] Update `jayess:canvas` XML image loading so local `<image src="...">`, `scrollbar-thumb`, and `scrollbar-track` use the generic image loader for supported local formats.
- [x] Update `packImage(path)` embedding to accept `.png`, `.jpeg`, `.jpg`, `.bmp`, `.psd`, `.gif`, and `.webp` without host tools, using generated C++ byte payloads and the same runtime decoder path.
- [x] Preserve deterministic diagnostics when `externals/stb/stb_image.h` or its license file is missing.
- [x] Add focused output tests proving `packImage()` accepts PNG byte assets and packages STB decoder/license artifacts.
- [x] Add generated C++ runtime tests under `test/` for small PNG, JPEG/JPG, BMP, GIF, and transparent PNG fixtures, verifying dimensions and selected pixels.
- [x] Add the focused libwebp adapter source list, generated-project source copying, and license packaging for real `loadWebp`, `decodeWebp`, and `packImage(".webp")` support.
- [x] Add generated C++ runtime tests under `test/` for small WebP fixtures after the libwebp adapter compiles through generated projects.
- [x] Update `docs/jayess-image-module.md`, `docs/jayess-canvas-module.md`, `docs/jayess-canvas-xml-scenes.md`, and `docs/standard-library.md` with the final WebP format matrix after those decoder adapters land.
