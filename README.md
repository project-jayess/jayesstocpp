...
- `jayess:canvas` for off-screen 2D drawing over image buffers and the planned Jayess-owned XML scene renderer
...
The current canvas implementation renders into an off-screen software image buffer and can save deterministic image files. Jayess ships an original license-safe default 5x7 bitmap font through `jayess:font`, and canvas text can select registered bitmap fonts, file-backed TTF/TrueType-style OTF/WOFF font handles, packaged fonts, or registered system default font handles with `font` / `fontFamily` options. System font discovery probes common host font paths at runtime and falls back to `jayess-default-5x7` when unavailable. TrueType-backed fonts use a Jayess-owned dependency-free glyph rasterizer; CFF/CFF2 and non-empty WOFF2 remain unsupported for real glyph pixels. Native UI work currently stays at the lower-level rendering layer: portable CPU rendering through `jayess:canvas`, live presentation through `jayess:window`, and optional acceleration through `jayess:gpu`. Canvas scene rendering accepts `backend: "auto"` by default and currently records a CPU fallback through `actualBackend(canvas)` until a real GPU XML renderer is implemented. There is no shipped `jayess:gui` standard-library module for now. HTML/CSS rendering is being removed from the canvas direction; XML scenes should describe shapes such as `<rectangle>`, `<ellipse>`, `<triangle>`, and `<capsule>` with attributes instead. XML scene canvases keep their normalized scene tree for runtime updates, so apps can mutate existing ID-bearing elements with `setAttribute`, derive `mouseover`/`mouseout` transitions with canvas `dispatchEvent`, and present the updated buffer through `jayess:window`. XML shadows use a CSS-like mask pipeline: shape alpha mask, spread, native alpha blur, tint, and alpha blit behind the source shape. GPU support should expose a Jayess-owned API while using focused backend adapters for platform graphics APIs such as Direct3D, Metal, Vulkan, or OpenGL where appropriate, rather than copying a broad third-party GUI, browser, or graphics toolkit into the repository. See [docs/jayess-canvas-xml-scenes.md](./docs/jayess-canvas-xml-scenes.md) for the planned XML scene schema.
...
See [Jayess.md](./Jayess.md), [docs/overview.md](docs/overview.md), [docs/stdlib-and-core-model.md](docs/stdlib-and-core-model.md), [docs/reachable-symbol-emission.md](docs/reachable-symbol-emission.md), [docs/internal-tools.md](docs/internal-tools.md), [docs/jayess-native-rendering.md](docs/jayess-native-rendering.md), [docs/testing.md](docs/testing.md), [docs/shared-library.md](docs/shared-library.md), [docs/limitations.md](docs/limitations.md), [docs/workflow.md](docs/workflow.md), and [docs/review-discipline.md](docs/review-discipline.md).
