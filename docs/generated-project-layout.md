...
| `jayess_reachability.json` | Export-level reachability summary for named import lists, retained/pruned export names, and whole-module reasons. |
...
For executable output, when the entry Jayess file defines a top-level `function main()`, the output also includes `executable/jayess_main.cpp` and `executable/jayess_executable.json`. The generated native entrypoint initializes the entry module, calls the generated Jayess `main()` function, and maps a numeric return value to the process exit code. Non-numeric or null returns exit with `0`; uncaught Jayess or C++ exceptions are reported on stderr and exit with `1`.
...
The generated-project direction is export-level reachable symbol emission for named import lists. Metadata should make this reviewable by recording requested import names, reachable exports, retained declarations, pruned declarations where practical, retained native artifacts, retained runtime fragments, and whole-module reasons for default imports, namespace imports, side-effect imports, mixed imports, and analysis fallbacks. See [reachable-symbol-emission.md](./reachable-symbol-emission.md).
...
- `jayess_reachability.json` lists each module's reachable, retained, and pruned export names plus whole-module reasons for entry modules, non-named import forms, ambiguous export-all forwarding, and top-level side-effect fallbacks. Generated C++ uses this data for conservative declaration pruning in safe modules. The focused `jayess:fs` text-helper slice can omit unrelated FS exports, dependency modules, native bridge headers, and optional runtime fragments when only `readTextSync` / `writeTextSync` are reachable. Other repository-owned `jayess:*` modules and native source/library artifacts still use broader fallback output until their focused pruning slices land.
...
## Output Planning Helpers

`transpileFile()` delegates focused output decisions to small helper modules:

- `src/output/emitted-module-plan.js` decides whether a module, module dependency edge, declaration set, import-local set, or native artifact is retained for generated output.
- `src/output/module-metadata-plan.js` builds deterministic module metadata, generated paths, namespaces, and export-name surfaces for modules that will be written.
- `src/output/native-artifacts.js` copies retained native headers, sources, shared libraries, and static libraries into the generated target directory while preserving path-safety diagnostics and retained-artifact metadata.

These helpers keep output planning separate from parser, semantic, module-resolution, and C++ emission logic.
