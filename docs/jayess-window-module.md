...
`addEventListener(window, name, callback)` registers a JavaScript-style callback for normalized window events. It is a convenience layer over `jayess:events`; it does not replace `pollEvents(window)`. `dispatchEvents(window)` is the low-level event pump: it drains the native event queue with `pollEvents(window)`, coalesces high-frequency `mouseMove`, `resize`, and `wheel` events for listener dispatch, emits each retained event by its `type`, and returns the emitted events. Wheel coalescing sums `deltaX` and `deltaY` and keeps the latest pointer position. `pollEvents(window)` remains the raw queue-drain helper for code that needs every native event. `removeEventListener(window, name, callback)` removes matching listeners by callback identity.
