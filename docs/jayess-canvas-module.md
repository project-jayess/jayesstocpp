...
`dispatchEvent(canvas, event)` currently derives `mouseover` and `mouseout` from `mouseMove` events. Keyboard focus, text input routing, creation of new UI elements, and higher-level controls remain later work.
