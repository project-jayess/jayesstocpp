...
- `tickTextCursor(canvas, nowMillis)`
...
Visual XML shapes can also render inner text labels. Use child text for readable markup, or update the same value at runtime with `setAttribute(canvas, id, "text", "New label")`. Label styling uses `padding`, `font-color`, `font-family`, `font-size`, `line-height`, `letter-spacing`, `word-spacing`, `text-transform`, `text-decoration`, `text-overflow`, `text-wrap`, `text-select`, `text-select-color`, `text-select-padding`, `text-select-corners`, `mouse-select`, `mouse-select-color`, `mouse-select-padding`, `mouse-select-corners`, `text-input`, `text-cursor`, `text-cursor-width`, `text-cursor-height`, `text-cursor-corners`, `text-cursor-speed`, `overflow`, `overflow-x`, `overflow-y`, `scrollbar-width`, and `scrollbar-color`; these can also be changed with `setAttribute` or `setAttributes`. `font-color` affects only the label text, while `fill` still controls the shape body. `text-select="start end"` highlights a persistent XML/runtime character range in the rendered label text, and `text-select="none"` clears that static range. The static `text-select` range is independent from mouse selection, so it continues to render after typing more text and after a separate mouse selection is made. `text-select-padding` / `mouse-select-padding` expand the highlight background in pixels, while `text-select-corners` / `mouse-select-corners` round that highlight background. `mouse-select="true"` lets canvas pointer events update a temporary mouse selection range by dragging, and `selectedText(canvas)` returns the current temporary mouse-selected text for app-managed clipboard shortcuts. Clicking outside selectable text clears only the temporary mouse selection, not the static `text-select` highlight. `text-input="true"` lets a shape receive focus on mouse click and mutate its label text from forwarded `keyDown` and `textInput` canvas events. Focused text inputs draw a caret using `text-cursor` color, `text-cursor-width`, optional `text-cursor-height`, and optional `text-cursor-corners`; caret placement follows wrapped lines and element scroll offsets. `Ctrl+Z` undoes focused text-input edits and `Ctrl+Shift+Z` redoes them when the forwarded key event includes `ctrlKey` / `shiftKey` state. Text edits are grouped into undo snapshots when more than 300ms passes between text-editing events. `text-cursor-speed="0"` keeps the caret visible; larger decimal values blink faster. Apps call `tickTextCursor(canvas, nowMillis)` with a monotonic millisecond value, normally from `jayess:window` `frame` events or `jayess:time.millis()`, and request a render when it returns `true`. `measureTextBox(canvas, text, rect, options)` returns the wrapped content width, height, line list, line widths, and line height used by `drawTextBox` and XML scrollbar decisions.
...
`reflowScene(canvas, xmlText, options)` reparses an XML scene and redraws the existing canvas while preserving runtime state for elements with stable `id` values. It recalculates layout geometry from the new XML, but keeps mutable UI state such as `text`, fill/outline paint, opacity/visibility, selection, focused text-input state, caret position, and element scroll offsets. Use this for resize-driven responsive layouts instead of replacing the canvas with a fresh `renderScene(...)` result. Dirty redraws from `setAttribute`, `setAttributes`, text input, scroll, and hover changes already operate on the existing runtime scene and preserve state.
...
`dispatchEvent(canvas, event)` currently derives `mouseover` and `mouseout` from `mouseMove` events, button state changes from pointer events, scroll changes from wheel and scrollbar drag events, `textselect` events for XML elements with `mouse-select="true"`, `focus` / `blur` events for XML elements with `text-input="true"`, and focused `textinput` events when key or text input changes the focused element text. Text-input focus is selected by left-clicking the top text-input element under the pointer. Clicking outside selectable text clears the temporary mouse selection. Creation of new UI elements and higher-level controls remain later work.
